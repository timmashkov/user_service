...
class PermissionWriteRepository(AbstractWriteRepository):
    ...
    async def create_item(self, **kwargs: Any) -> Optional[Permission]:
    ...
    async def update_item(self, **kwargs: Any) -> Optional[Permission]:
        async with self._session() as session:
            stmt = (
                update(self._model)
                .values(kwargs)
    ...
    async def delete_item(self, **kwargs: Any) -> Optional[Permission]:
        async with self._session() as session:
            stmt = (
                delete(self._model)
                .where(self._model.uuid == kwargs["uuid"])
